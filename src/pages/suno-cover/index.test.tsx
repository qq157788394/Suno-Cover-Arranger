...
describe("SunoCover Component", () => {
  ...
  beforeEach(() => {
    ...
    (AIProviderFactory.createProvider as jest.Mock).mockReturnValue(
      mockDeepSeekProvider
    );
  ...
  it("should show loading state when generating", async () => {
    ...
    mockDeepSeekProvider.generate.mockResolvedValue({
      success: true,
      content: "Test styles\n\nTest lyrics",
    ...
    const targetArtistInput =
      screen.getByPlaceholderText("例如：张惠妹、陈奕迅、周杰伦……");
    ...
    await waitFor(() => {
      expect(mockDeepSeekProvider.generate).toHaveBeenCalled();
    });
  });
